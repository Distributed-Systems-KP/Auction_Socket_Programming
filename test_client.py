...
def seller_client(sock):
    try:
        ...
        if len(auction_details) != 4:
            print("Invalid input format. Please enter exactly four values.")
            return

        auc_type, auc_min_price, max_bids, item_name = auction_details

        # Validate inputs
        if not (auc_type.isdigit() and auc_min_price.isdigit() and max_bids.isdigit()):
            print("Auction type, minimum price, and maximum number of bids must be integers.")
            return
        ...
        print(f"Error in seller client: {e}")
